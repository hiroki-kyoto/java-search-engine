...
import net.token.searcher.AutoArrayUnit;
...
public class AutoArray<T>{
  ...
  public T pop() throws Exception{
    if(length==1){
      ...
      tl.log("Error happens in AutoArray: Array is already empty! Can't pop Element! --this kind of exception is safe, won't affect the main thread.");
    }
    T temp = tail.getData();
    tail = tail.prev();
    length--;
