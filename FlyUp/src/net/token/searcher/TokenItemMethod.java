...
import net.token.searcher.AutoArray;

public class TokenItemMethod{
	static TokenLog tl = new TokenLog();
  ...
  public void getTokenItemsFromDataBase(String mssqlUrl, String username, 
    ...
    catch(Exception e){tl.log("happend in TokenItemMethod: Cannot find the sqlserver jdbc!无法找到相关的数据库驱动！");}
    ...
    try {
    	...
    	for(int num=0; num<tableNames.length; num++){
    		...
    		try{
    			...
    			tl.log("Error in Statement! " + e.getMessage());
    		} 
    	}
    } 
    catch (SQLException e) {
     tl.log("Could not connect! " + e.getMessage());
    }
  }
